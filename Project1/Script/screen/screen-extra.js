...
var DictionaryScreen = defineObject(BaseScreen,
{
	...
	_setMessagePagerData: function() {
		...
		messagePagerParam.maxWidth = 24 * size;
		messagePagerParam.rowCount = Math.floor(this.getContentHeight() / 28);
		messagePagerParam.interval = 10;
		messagePagerParam.isScrollLocked = true;
	...
	_changeNewPage: function(isTarget) {
		...
		this._messagePager.setMessagePagerText(data.getPageText(this._storyDataChanger.getTruePageIndex()));
	},
...
var CharacterScreen = defineObject(DictionaryScreen,
{
	...
	drawDictionaryImage: function(obj) {
		...
		if (image !== null) {
			...
			y = this._getIllustY(image);
			image.draw(x, y);
		}
		else {
			x = this._getFaceX(image);
			y = this._getFaceY(image);
	...
	_getFaceX: function(image) {
		return this.getContentX() + 405;
	},
	
	_getFaceY: function(image) {
		return this.getContentY();
	},
	
	_getIllustX: function(image) {
		return this.getContentX() + 250;
	},
	
	_getIllustY: function(image) {
		var y;
...
var WordScreen = defineObject(DictionaryScreen,
{
	...
	drawDictionaryImage: function(obj) {
		...
		if (image !== null) {
			x = this._getPictureX(image);
			y = this._getPictureY(image);
			image.draw(x, y);
		}
	},
	...
	_getPictureX: function(image) {
		return this.getContentX() + 375;
	},
	
	_getPictureY: function(image) {
		return this.getContentY() + 10;
	}
}
);
