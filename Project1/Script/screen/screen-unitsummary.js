...
var UnitSummaryScrollbar = defineObject(BaseScrollbar,
{
	...
	_drawLeft: function(x, y, unit, isSelect, index) {
		...
		TextRenderer.drawKeywordText(x + 50, y + 15, unit.getName(), -1, color, font);
		
		NumberRenderer.drawRightNumberColor(x + 170, y + 15, unit.getId(), this._getColorIndexFromUnit(unit), 255);
	},
	...
	_drawRight: function(x, y, unit, isSelect, index) {
		...
		for (i = 0; i < count; i++) {
			...
			TextRenderer.drawKeywordText(x + dx, y + 15, arr[i], -1, color, font);
			dx += this._getStateInterval();
		}
	},
