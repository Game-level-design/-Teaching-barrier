...
var UnitAllCommandEventCommand = defineObject(BaseEventCommand,
{
	...
	isEventCommandSkipAllowed: function() {
		// 「ダメージを与える」や「アイテムの増減」などはスキップを無効にできるため、許可しない
		return false;
	},
	...
	_completeEventCommandMemberData: function() {
		...
		if (root.getEventCommandObject().getAnimePlayType() === AnimePlayType.SYNC) {
...
var AllEventFlowEntry = defineObject(BaseFlowEntry,
{
	...
	_itemChange: function(generator, targetUnit, command) {
		generator.unitItemChange(targetUnit, command.getTargetItem(), command.getIncreaseValue(), this._isCommandSkip(command));
	},
	...
	_damagehit: function(generator, targetUnit, command) {
		generator.damagehit(targetUnit, command.getDamageAnime(), command.getDamageValue(), command.getDamageType(), command.getLaunchUnit(), this._isCommandSkip(command));
	},
	...
	_unitStateAddition: function(generator, targetUnit, command) {
		generator.unitStateAddition(targetUnit, command.getStateInvocation(), command.getIncreaseValue(), command.getLaunchUnit(), this._isCommandSkip(command));
	},
	...
	_mapPosOperation: function(generator, targetUnit, command) {
		generator.mapPosOperation(targetUnit, targetUnit.getMapX(), targetUnit.getMapY(), command.getMapChipResourceHandle());
	},
