...
// RealBattleもEasyBattleも、
// どのユニットから攻撃するかなどの順番は同一である。
// こうした順番は、AttackOrder内のAttackEntryで構成されており、
// それを確認するのがAttackFlowになる。
var AttackFlow = defineObject(BaseObject,
{
	...
	_doAttackAction: function() {
		...
		for (i = 0; i < count; i++) {
			StateControl.arrangeState(active, activeStateArray[i], IncreaseType.INCREASE);
		}
		
		count = passiveStateArray.length;
		for (i = 0; i < count; i++) {
			StateControl.arrangeState(passive, passiveStateArray[i], IncreaseType.INCREASE);
		}
		
		if (isItemDecrement) {
			// 攻撃する側の武器を減らす。
			// ここで、アイテムが破棄されるようなことはない。
			ItemControl.decreaseLimit(active, ItemControl.getEquippedWeapon(active));
		}
	},
