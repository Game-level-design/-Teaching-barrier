...
var UnitSentenceWindow = defineObject(BaseWindow,
{
	...
	setUnitData: function(unit) {
		...
		for (i = 0; i < count; i++) {
			...
			this._groupArray[i].setCalculatorValue(unit, ItemControl.getEquippedWeapon(unit), this._totalStatus);
		}
	},
	...
	drawWindowContent: function(x, y) {
		...
		for (i = 0; i < count; i++) {
			this._groupArray[i].drawUnitSentence(x, y, this._unit, weapon, this._totalStatus);
