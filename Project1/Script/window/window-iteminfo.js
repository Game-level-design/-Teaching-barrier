...
ItemSentence.Skill = defineObject(BaseItemSentence,
{
	drawItemSentence: function(x, y, item) {
		...
		if (list.getTypeCount() === 0) {
			return 0;
		}
		
		ItemInfoRenderer.drawKeyword(x, y, root.queryCommand('skill_object'));
		x += ItemInfoRenderer.getSpaceX();
		ItemInfoRenderer.drawList(x, y, list);
	},
	
	getItemSentenceCount: function(item) {
		var list = item.getSkillReferenceList();
		
		return list.getTypeCount();
	}
}
);
...
var ItemInfoRenderer = {
	...
	drawList: function(x, y, list) {
		...
		// このため、名前の数だけ行も増えていく。
		for (i = 0 ; i < count; i++) {
			TextRenderer.drawKeywordText(x, y, list.getTypeData(i).getName(), -1, color, font);
			y += this.getSpaceY();
		}
	},
	...
	getStateCount: function(stateGroup) {
		...
		if (stateGroup.isAllBadState()) {
			...
			count = stateGroup.getStateReferenceList().getTypeCount();
		}
