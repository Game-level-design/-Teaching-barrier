...
var StatusScrollbar = defineObject(BaseScrollbar,
{
	...
	setStatusFromParam: function(typeArray, paramArray, col, row) {
		...
		this._statusArray = new Array(typeArray.length);
		
		for (i = 0; i < count; i++) {
			statusEntry = StructureBuilder.buildStatusEntry();
			statusEntry.type = typeArray[i];
			statusEntry.param = paramArray[i];
			statusEntry.bonus = 0;
			statusEntry.index = i;
			statusEntry.isRenderable = false;
			this._statusArray[i] = statusEntry;
		}
		
		this.setScrollFormation(col, row);
