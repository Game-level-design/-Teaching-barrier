...
var FaceView = defineObject(BaseObject,
{
	...
	drawMessageView: function() {	
		...
		if (this._topView !== null) {
			this._topView.drawCharIllust();
		}
		
		if (this._centerView !== null) {
			this._centerView.drawCharIllust();
		}
		
		if (this._bottomView !== null) {
			this._bottomView.drawCharIllust();
		}
		
		// 次にメッセージを描画
		
		if (this._topView !== null) {
			this._topView.drawMessageView();
		}
		
		if (this._centerView !== null) {
			this._centerView.drawMessageView();
		}
		
		if (this._bottomView !== null) {
			this._bottomView.drawMessageView();
		}
	},
...
var TeropView = defineObject(BaseMessageView,
{
	drawMessageView: function() {
		...
		this.drawCharIllust();
		BaseMessageView.drawMessageView.call(this);
	}
}
);
...
var BacklogScrollbar = defineObject(BaseScrollbar,
{
	...
	_getNameColor: function(command, textui) {
		...
		raceId = unit.getClass().getRace().getId();
		if (raceId === 0) {
